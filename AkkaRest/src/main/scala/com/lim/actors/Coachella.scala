package com.lim.actors

import akka.actor._
import akka.pattern.{ask, pipe}
import akka.util.Timeout
import com.lim.messages.Coachella._
...
class Coachella extends  Actor {
  import com.lim.messages.TicketSeller
  ...
  def receive: PartialFunction[Any, Unit] = {
    ...
    case GetEvents ⇒
      ...
      def convertToEvents(f: Future[Iterable[Option[Event]]]): Future[Events] = {
        f.map(_.flatMap(_.toList)).map(l ⇒ Events(l.toVector))
      }
